...
import { ShieldCheck, MapPin, LogIn, Settings2, Plus, X, Zap, Loader2 } from "lucide-react"
...
export function OrdersSidebar({ popularCategories }: OrdersSidebarProps) {
  ...
  // 1. ПОЛУЧАЕМ КОНТЕКСТ (Шина данных: город, слаг)
  ...
  // 2. ПРОФИЛЬ И СКИЛЛЫ (Берем из кеша через твой хук)
  const { profile, hasSkills } = useUserSkills()

  // 3. МУТАЦИЯ УДАЛЕНИЯ С КАСКАДНЫМ ОБНОВЛЕНИЕМ
  const { mutate: handleRemoveSkill, variables: deletingId } = useMutation({
    ...
    onMutate: async (categoryId) => {
      // Оптимистично убираем скилл из профиля
    ...
    onSuccess: () => {
      // ВАЖНО: Инвалидируем заказы, чтобы сервер пересчитал ленту под новый набор скиллов
    ...
    <div className="space-y-6">
      ...
      {profile ? (
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1">
                <h4 className="font-black uppercase italic text-sm truncate">
                  {profile.user.name?.split(' ')[0] || 'Партнер'}
                </h4>
        ...
        <div className="bg-blue-600 rounded-[2.5rem] p-8 text-white shadow-xl shadow-blue-100 relative overflow-hidden">
          <h4 className="text-xl font-black uppercase italic tracking-tighter mb-4 leading-none">
      ...
      <div className="bg-white border border-slate-100 p-8 rounded-[2.5rem] shadow-sm space-y-6">
        <div className="flex items-center justify-between">
          ...
          {profile && (
            <button 
                ...
                className="p-2 bg-slate-50 rounded-xl text-blue-600 hover:bg-blue-600 hover:text-white transition-all active:scale-95"
            >
        ...
        <div className="flex flex-wrap gap-2">
          {profile && hasSkills ? (
            profile.skills.map((skill) => {
              ...
              return (
                <div key={skill.categoryId} className={cn(
                  "flex items-center gap-2 bg-slate-50 border border-slate-100 px-3 py-1.5 rounded-xl text-[10px] font-black text-slate-900 uppercase italic transition-opacity",
            ...
            <button onClick={openCatModal} className="text-[10px] font-bold text-blue-600 uppercase italic hover:underline p-1">
              + Выберите категории в профиле
            </button>
          )}
        </div>
      </div>

      {/* 3. ПОПУЛЯРНЫЕ КАТЕГОРИИ (ТРЕНДЫ) */}
        ...
        <div className="flex flex-col gap-2 relative">
          {popularCategories.map((cat) => (
            <Link 
                ...
                className="flex items-center justify-between group py-1 border-b border-transparent hover:border-slate-50 transition-all"
            >
