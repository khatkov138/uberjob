...
import {
  ...
  Zap, ChevronRight, Check, Plus, X, Wallet, Banknote
} from "lucide-react"
...
export function OrderCard({ order, userCategories = [], isMatched, toggleCategory }: any) {
  ...
  // В будущем данные будут приходить из order.client
  const clientStats = {
    name: order.client?.name || "khatkov138",
    projects: 3,
    hireRate: 0
  }
  ...
  return (
    <div className="relative group">
      {/* 1. БЕЙДЖ СМАРТ-ПОДБОРА (уменьшен) */}
      {isMatched && (
      ...
      <Card className={cn(
        ...
        {/* Уменьшили отступы с p-10 до p-7 */}
        <CardContent className="p-6 md:p-7">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start gap-4 mb-1">
            <div className="space-y-3 flex-1 w-full">
              <Link href={`/pro/orders/${order.id}`} className="block">
                {/* Уменьшили заголовок с 5xl до 3xl */}
                  ...
                  <span
                    ...
                    className={cn(
                      ...
                      userCategories.includes(cat)
                        ? "text-blue-600" // Твой навык - подсвечен
                        : "text-slate-300" // Чужой навык - серый
                    )}
                  >
                    #{cat.replace(/\s+/g, '')}
                  </span>
          ...
          {/* 3. ОПИСАНИЕ (стало компактнее) */}
          <div className="mt-5 mb-6 pl-4 border-l-2 border-slate-100">
            <p className="text-base text-slate-500 font-medium italic lowercase leading-snug line-clamp-2 md:line-clamp-3">
              {order.description}
            </p>
          </div>
          ...
          <div className="flex flex-wrap gap-2 mb-6">
            <div className="flex items-center gap-1.5 px-3 py-1 bg-orange-50 rounded-lg border border-orange-100 text-orange-500">
              <Zap className="w-3 h-3 fill-current" />
              <span className="text-[9px] font-black uppercase italic">Срочно</span>
            </div>
            ...
            <div className="flex items-center gap-6">
              <div className="flex items-center gap-3">
                <div className="w-10 h-10 rounded-full bg-slate-900 flex items-center justify-center text-white font-black text-sm shadow-md">
                  {clientStats.name.charAt(0).toUpperCase()}
                </div>
                <div>
                  <div className="text-[8px] font-black text-slate-300 uppercase leading-none mb-0.5 tracking-widest">Заказчик</div>
                  <div className="text-sm font-black text-slate-900 tracking-tight">{clientStats.name}</div>
                </div>
              </div>
              ...
              <div className="flex gap-6 border-l border-slate-100 pl-6">
                <div className="text-center">
                  ...
                  <div className="text-sm font-black text-slate-900">{clientStats.projects}</div>
                </div>
                <div className="text-center">
                  <div className="text-[8px] font-black text-slate-300 uppercase leading-none mb-0.5">Наем</div>
                  <div className="text-sm font-black text-slate-900">{clientStats.hireRate}%</div>
                </div>
              </div>
            </div>
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
              <div className="flex flex-wrap items-center gap-5 text-slate-400 min-w-0">
                <div className="flex items-center gap-1.5 shrink-0">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-widest">14м назад</span>
                </div>
                <div className="flex items-center gap-1.5 shrink-0">
                  <MessageSquare className="w-4 h-4 text-slate-200" />
                  <span className="text-[10px] font-black uppercase tracking-widest">3 отклика</span>
                </div>
                {/* ЛОКАЦИЯ С ЗАЩИТОЙ ОТ ПЕРЕПОЛНЕНИЯ */}
                <div className="flex items-center gap-1.5 min-w-0">
                  <MapPin className="w-4 h-4 text-slate-200 shrink-0" />
                  <span className="text-[10px] font-black uppercase tracking-widest italic truncate max-w-[120px] md:max-w-[200px]">
                    {order.address}
                  </span>
                </div>
              </div>

              {/* КНОПКА: ЕДИНСТВЕННЫЙ ЭЛЕМЕНТ С HOVER-ЭФФЕКТОМ */}
              <Link
