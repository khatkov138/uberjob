...
export function CitySearchInput({ defaultValue, onSelect, placeholder }: CitySearchInputProps) {
    const [query, setQuery] = React.useState(defaultValue || "")
    const [suggestions, setSuggestions] = React.useState<CitySuggestItem[]>([])
    ...
    React.useEffect(() => {
        ...
        const fetchCities = async () => {
            ...
            setIsLoading(true)
            try {
                // Стучимся на выделенный эндпоинт поиска городов
                const data = await handleApi<CitySuggestItem[]>(
                    fetch(`/api/geo/suggest?text=${encodeURIComponent(debouncedQuery)}`)
                )
                setSuggestions(data || [])
            } catch (err) {
                console.error("CITY_SUGGEST_ERROR:", err)
            } finally {
                setIsLoading(false)
            }
        }
    ...
    const handleItemClick = (item: CitySuggestItem) => {
        isFirstRender.current = false
