...
export function ChatWindow({ recipientId, recipientName, currentUserId, orderId }: ChatWindowProps) {
  ...
  // ФИНАЛЬНЫЙ СКРОЛЛ: Пробивает любые лаги отрисовки
  const scrollToBottom = (behavior: ScrollBehavior = "auto") => {
    if (!scrollRef.current) return
    const el = scrollRef.current
    const perform = () => { el.scrollTop = el.scrollHeight + 2000 }

    perform() // Мгновенно
    requestAnimationFrame(perform) // В следующем кадре
    setTimeout(perform, 50) // Через 50мс для надежности
  }
      ...
      queryClient.setQueryData<InfiniteData<InfiniteMessagesResponse>>(queryKey, (old) => {
        ...
        return {
          ...
          pages: old.pages.map((page, index) => {
            if (index === 0) {
              const cleanMessages = page.messages.filter((m) => {
                const isGhost = (m as ChatMessage).isOptimistic
                const isSame = m.text.trim() === newMessage.text.trim()
                return !(isGhost && isSame)
              })
  ...
  // ВОССТАНОВЛЕНИЕ ПОЗИЦИИ (Фикс активного якоря при входе)
  React.useLayoutEffect(() => {
    const container = scrollRef.current
    if (!container || isLoading || messages.length === 0) return

    const serverUnread = (data?.pages[0] as any)?.totalUnread || 0
    ...
    const timer = setTimeout(() => {
      ...
      if (stick || savedPos === undefined) {
        ...
        // Даем небольшой допуск (30px) чтобы якорь не вылезал из-за паддингов
  ...
  const handleScroll = (e: React.UIEvent<HTMLDivElement>) => {
    ...
    isStickToBottom[chatKey] = fromBottom < 80
    // Увеличили порог до 35px, чтобы якорь не "мерцал" на пустом месте
  ...
  const mutation = useMutation({
    ...
    onSuccess: () => queryClient.invalidateQueries({ queryKey: ["dialogs"] }),
    ...
    <div className="flex flex-col h-full w-full bg-white overflow-hidden relative">
      <div className="px-10 py-6 border-b border-slate-100 flex items-center justify-between shrink-0 bg-white z-20">
        <Link href={`/profile/${recipientId}`} className="flex items-center gap-4 group">
          ...
          <div className="text-left">
            ...
            <h2 className="text-lg font-black uppercase italic text-slate-900 truncate max-w-[200px]">{recipientName}</h2>
          </div>
        </Link>
        {orderId && (
      ...
      <div className="flex-1 relative bg-slate-50/20 overflow-hidden flex flex-col">
        <div
          ...
          className="absolute inset-0 overflow-y-auto p-6 md:p-10 chat-scrollbar"
        ...
        >
          ...
          {isLoading ? (
            ...
            <div className="flex flex-col gap-6 messages-wrapper pb-10">
              {messages.map((msg) => {
                const isUnreadByMe = String(msg.senderId) === String(recipientId) && !msg.isRead
                return (
                  <div key={msg.id} className={cn("flex flex-col max-w-[85%] md:max-w-[75%] shrink-0", msg.senderId === currentUserId ? "ml-auto items-end" : "items-start")}>
                    <div className={cn(
                      ...
                      msg.senderId === currentUserId
                        ...
                        : isUnreadByMe ? "bg-blue-50 border-2 border-blue-100 text-slate-900 rounded-tl-none" : "bg-white border-2 border-slate-50 text-slate-900 rounded-tl-none shadow-sm"
                    )}>
                      {msg.text}
                    </div>
                    <div className="flex items-center gap-2 mt-2 px-3 opacity-40 shrink-0">
      ...
      <div className="p-6 md:p-8 bg-white border-t border-slate-100 shrink-0">
        <form onSubmit={(e) => { e.preventDefault(); if (input.trim() && !mutation.isPending) mutation.mutate(input) }} className="flex gap-4 items-center bg-slate-50 p-2 rounded-[2.5rem] border-2 border-transparent focus-within:border-blue-600 focus-within:bg-white shadow-inner transition-all">
          <input value={input} onChange={(e) => setInput(e.target.value)} disabled={mutation.isPending} placeholder="НАПИШИТЕ СООБЩЕНИЕ..." className="flex-1 h-14 px-8 bg-transparent outline-none font-black italic text-[11px] tracking-widest text-slate-900" />
